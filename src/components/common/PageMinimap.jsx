    ...
    <div className="fixed right-3 top-1/2 -translate-y-1/2 z-30 hidden lg:block" style={{ width: 32 }}>
        ...
        >
            ...
            return (
              ...
              >
                <div className="absolute inset-0" style={{
                  background: isActive ? 'rgba(224,53,85,0.15)' : i % 2 === 0 ? 'rgba(22,34,54,0.6)' : 'rgba(14,26,40,0.6)',
                  borderLeft: isActive ? '2px solid rgba(224,53,85,0.8)' : '2px solid transparent',
                }} />
                {height > 14 && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
                    {b.iconSrc ? (
                      ...
                      <span className="text-[9px] font-bold"
                        style={{ color: isActive ? 'rgba(224,53,85,0.9)' : 'rgba(255,255,255,0.18)' }}>
                        {b.abbrev}
                      </span>
                    )}
                  </div>
                ...
                {(hoveredId === b.id || isActive) && (
                  <div className={`absolute right-full top-1/2 -translate-y-1/2 mr-3 px-2.5 py-1 rounded-md text-xs font-medium whitespace-nowrap pointer-events-none shadow-lg border ${
                    isActive ? 'bg-navy-800 border-ob-red/50 text-white' : 'bg-navy-800 border-navy-600 text-steel-300'
                  }`}>{b.label}</div>
                )}
              </div>
            );
          ...
          <div className="absolute left-0 right-0 pointer-events-none rounded-sm" style={{
            ...
            background: 'rgba(224,53,85,0.12)', border: '1px solid rgba(224,53,85,0.35)',
          }} />
        </div>
    </div>
  );
